...
export async function getAlbumsByArtistId(artistId: string) {
  try {
    const response = await fetch(
      `${PUBLIC_AUDIODB_URL}/album.php?i=${artistId}`
    );
...
export async function getMvidsByArtistId(artistId: string) {
  try {
    const response = await fetch(
      `${PUBLIC_AUDIODB_URL}/mvid.php?i=${artistId}`
    );
    const data = await response.json();
...
export async function getArtistInfo(artistName: string) {
  ...
  const lastInfos = CACHE.get(artist.id);
  console.log('infos cache', lastInfos);
  if (lastInfos) return lastInfos;
  ...
  const albumsMap = new Map(
    allAlbums.album.map((album) => [album.idAlbum, album])
  );
